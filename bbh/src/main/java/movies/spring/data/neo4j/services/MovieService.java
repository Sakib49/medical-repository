...
public class MovieService {
	...
	private Map<String, Object> toD3Format(Collection<Disease> movies) {
		List<Map<String, Object>> nodes = new ArrayList<>();
		List<Map<String, Object>> rels = new ArrayList<>();
		int i = 0;
		Iterator<Disease> result = movies.iterator();
		while (result.hasNext()) {
			Disease movie = result.next();
    ...
    public Disease findByTitle(String title) {
        Disease result = movieRepository.findByTitle(title);
        return result;
    }

    @Transactional(readOnly = true)
    public Collection<Disease> findByTitleLike(String title) {
        Collection<Disease> result = movieRepository.findByTitleLike(title);
        return result;
    }

	@Transactional(readOnly = true)
	public Map<String, Object>  graph(int limit) {
		Collection<Disease> result = movieRepository.graph(limit);
